...
export function useDropzone() {
    ...
    const handleDrop = (event: DragEvent<HTMLDivElement>): FileReturnType => {
        ...
        const isDocUploaded = isFileTypeSelected(files, ACCEPTED_DOC_TYPES);

        if (isImageUploaded) return handleImageFiles(files);
        if (isDocUploaded) return handleDocFiles(files);
    ...
    const handleDocFiles = (files: FileList): FileReturnType => {
        const acceptedFiles: ExtendedFile[] = [];
        const rejectedFiles: ExtendedFile[] = [];
        ...
        if (rejectedFiles.length > 0) {
            ...
            return {
                status: "success",
                message: "Document files uploaded",
                isSuccess: true,
                isError: false,
                data: {
                    acceptedFiles,
    ...
    return {
        ...
        uploadConfig: {
            ...
            maxDocFileSize: MAX_DOC_FILE_SIZE,
            acceptedDocTypes: ACCEPTED_DOC_TYPES,
        },
    };
}
